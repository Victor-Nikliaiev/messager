...
from assets.ui.ui_info import Ui_Form
...
class QCustomTipOverlay(QWidget, Ui_Form):
    ...
    def __init__(
        ...
        if toolFlag:
            self.setWindowFlags(
                self.windowFlags() | Qt.Popup | Qt.Tool | Qt.FramelessWindowHint
            )
        else:
    ...
    def moveButton(self):
        ...
        margins = self.layout().contentsMargins()
        x, y = margins.right() * 5, margins.top() / 2
        w, h = self.width(), self.height()
...
class LeftTopTailQCustomQToolTipManager(LeftTailQCustomQToolTipManager):
    ...
    def draw(self, tipOverlay, painter, path):
        ...
        path.addPolygon(
            QPolygonF(
                [
                    QPointF(margins.right() / 2, margins.top()),
                    QPointF(0, margins.top() + 7),
        ...
        path.addPolygon(
            QPolygonF(
                [
                    ...
                    QPointF(0, h - margins.top() - 7),
                    QPointF(margins.right() / 2, h - margins.top() - 14),
                ]
            )
        )
...
class RightTopTailQCustomQToolTipManager(RightTailQCustomQToolTipManager):
    ...
    def draw(self, tipOverlay, painter, path):
        ...
        path.addPolygon(
            QPolygonF(
                [
                    QPointF(w - margins.right() / 2, margins.bottom()),
                    QPointF(w, margins.top() + 7),
                    QPointF(w - margins.right() / 2, margins.bottom() + 14),
                ]
            )
        )
...
class RightBottomTailQCustomQToolTipManager(RightTailQCustomQToolTipManager):
    ...
    def draw(self, tipOverlay, painter, path):
        ...
        path.addPolygon(
            QPolygonF(
                [
                    QPointF(w - margins.right() / 2, h - margins.bottom()),
                    QPointF(w, h - margins.bottom() - 7),
                    QPointF(w - margins.right() / 2, h - margins.bottom() - 14),
                ]
            )
        )
...
class QCustomQToolTipFilter(QObject):
    ...
    def showCustomToolTip(self, text, target):
        if (
            not text
            or hasattr(target, "customTooltip")
            and target.customTooltip is not None
        ):
            return
        target.customTooltip = QCustomTipOverlay(
            text=text,
            target=target,
